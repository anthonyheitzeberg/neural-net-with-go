...
func main() {
	...
	// Define the configuration for our neural network.
	config := neuralNetConfig{
		inputNeurons:  4,
		outputNeurons: 3,
		hiddenNeurons: 3,
	...
	netwrok := newNetwork(config)
	if err := netwrok.train(inputs, labels); err != nil {
		log.Fatal(err)
	}
	...
	predictions, err := netwrok.predict(testInputs)
	if err != nil {
		log.Fatal(err)
	}
